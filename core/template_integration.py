...
def write_autorun_main_c(
    *,
    ...
    _game_cfg = (project_data or {}).get("game") or {} if isinstance(project_data, dict) else {}
    ...
    if has_waves and has_enemy:
        # Sync so existing projects pick up engine fixes to the optional module
        # (e.g. the NGPC_WAVE_ENTRY_T guard that pairs with scenes_autogen.h to
        # prevent "Redeclaration of NgpcWaveEntry" when both headers land in
        # the same translation unit).
        _sync_optional_module(template_root, "optional/ngpc_wave")
    ...
    if has_shooting and has_enemy:
        ...
        c.append('    for (i = 0; i < (u8)NGPNG_AUTORUN_MAX_PLAYER_SHOTS; ++i) {\n')
        c.append('        /* PERF: cache &shots[i] in a local pointer so each s->field access\n')
        c.append('         * skips the per-access `shots + i*sizeof()` recompute that the\n')
        c.append('         * cc900 codegen would otherwise emit for every shots[i].xxx.\n')
        c.append('         * Decls go at top of for-body without initialiser, then assign\n')
        c.append('         * after the `active` guard (cc900 nested-decl-with-init bug). */\n')
        c.append('        NgpngPlayerShot *s;\n')
        c.append('        NgpngEnemy *e;\n')
        c.append('        u8 j;\n')
        c.append('        s16 sx_chk;\n')
        c.append('        s16 sy_chk;\n')
        c.append('        u8 _bn_hit;\n')
        c.append('        s16 _bn_probe;\n')
        c.append('        u8 _bn_t1;\n')
        c.append('        u8 _bn_t2;\n')
        ...
        c.append('            _bn_hit = 0u;\n')
        c.append('            if (s->world_x < cam_px) {\n')
        c.append('                s->world_x = cam_px;\n')
        c.append('                _bn_hit = 1u;\n')
        c.append('            } else if (s->world_x > (s16)(cam_px + 152)) {\n')
        c.append('                s->world_x = (s16)(cam_px + 152);\n')
        c.append('                _bn_hit = 1u;\n')
        c.append('            }\n')
        c.append('            if (!_bn_hit && sc->tilecol && sc->map_w > 0u && sc->map_h > 0u) {\n')
        c.append('                _bn_probe = (s->vx > 0)\n')
        c.append('                    ? (s16)(s->world_x + s->hb_x + s->hb_w - 1)\n')
        c.append('                    : (s16)(s->world_x + s->hb_x);\n')
        c.append('                _bn_t1 = ngpng_tilecol_world(sc, _bn_probe, (s16)(s->world_y + s->hb_y), TILE_PASS);\n')
        c.append('                _bn_t2 = ngpng_tilecol_world(sc, _bn_probe, (s16)(s->world_y + s->hb_y + s->hb_h - 1), TILE_PASS);\n')
        c.append('                if (_bn_t1 == TILE_SOLID || _bn_t2 == TILE_SOLID || ngpng_tile_blocks_left(_bn_t1) || ngpng_tile_blocks_right(_bn_t1) || ngpng_tile_blocks_left(_bn_t2) || ngpng_tile_blocks_right(_bn_t2)) {\n')
        c.append('                    s->world_x = (s16)(s->world_x - s->vx);\n')
        c.append('                    _bn_hit = 1u;\n')
        c.append('                }\n')
        c.append('            }\n')
        c.append('            if (_bn_hit) {\n')
        c.append('                s->vx = (s8)(-s->vx);\n')
        c.append('                if (s->bounce_sfx != 0xFFu) Sfx_Play(s->bounce_sfx);\n')
        c.append('                s->bounce_count = (u8)(s->bounce_count + 1u);\n')
        c.append('                if (s->bounce_max != 0u && s->bounce_count > s->bounce_max) {\n')
        c.append('                    ngpng_player_shot_kill(shots, active_count, i);\n')
        c.append('                    continue;\n')
        c.append('                }\n')
        c.append('            }\n')
        c.append('        }\n')
        c.append('        if (s->bounce_flags & 2u) {\n')
        c.append('            _bn_hit = 0u;\n')
        c.append('            if (s->world_y < cam_py) {\n')
        c.append('                s->world_y = cam_py;\n')
        c.append('                _bn_hit = 1u;\n')
        c.append('            } else if (s->world_y > (s16)(cam_py + 144)) {\n')
        c.append('                s->world_y = (s16)(cam_py + 144);\n')
        c.append('                _bn_hit = 1u;\n')
        c.append('            }\n')
        c.append('            if (!_bn_hit && sc->tilecol && sc->map_w > 0u && sc->map_h > 0u) {\n')
        c.append('                _bn_probe = (s->vy > 0)\n')
        c.append('                    ? (s16)(s->world_y + s->hb_y + s->hb_h - 1)\n')
        c.append('                    : (s16)(s->world_y + s->hb_y);\n')
        c.append('                _bn_t1 = ngpng_tilecol_world(sc, (s16)(s->world_x + s->hb_x), _bn_probe, TILE_PASS);\n')
        c.append('                _bn_t2 = ngpng_tilecol_world(sc, (s16)(s->world_x + s->hb_x + s->hb_w - 1), _bn_probe, TILE_PASS);\n')
        c.append('                if (_bn_t1 == TILE_SOLID || _bn_t2 == TILE_SOLID || ngpng_tile_blocks_ceiling(_bn_t1) || ngpng_tile_blocks_ceiling(_bn_t2) || ngpng_tile_has_floor(sc, _bn_t1) || ngpng_tile_has_floor(sc, _bn_t2)) {\n')
        c.append('                    s->world_y = (s16)(s->world_y - s->vy);\n')
        c.append('                    _bn_hit = 1u;\n')
        c.append('                }\n')
        c.append('            }\n')
        c.append('            if (_bn_hit) {\n')
        c.append('                s->vy = (s8)(-s->vy);\n')
        c.append('                if (s->bounce_sfx != 0xFFu) Sfx_Play(s->bounce_sfx);\n')
        c.append('                s->bounce_count = (u8)(s->bounce_count + 1u);\n')
        c.append('                if (s->bounce_max != 0u && s->bounce_count > s->bounce_max) {\n')
        c.append('                    ngpng_player_shot_kill(shots, active_count, i);\n')
        ...
        c.append('            }\n')
        c.append('        }\n')
        c.append('        sx_chk = (s16)(s->world_x - cam_px);\n')
        c.append('        sy_chk = (s16)(s->world_y - cam_py);\n')
        c.append('        if (s->bounce_flags == 0u && (sx_chk < -24 || sx_chk > 184 || sy_chk < -24 || sy_chk > 184)) {\n')
        c.append('            ngpng_player_shot_kill(shots, active_count, i);\n')
        c.append('            continue;\n')
        c.append('        }\n')
        ...
        c.append('            e = &enemies[j];\n')
        c.append('            ax = (s16)(s->world_x + s->hb_x);\n')
        c.append('            ay = (s16)(s->world_y + s->hb_y);\n')
        c.append('            bx = (s16)(e->world_x + e->hb_x);\n')
        c.append('            by = (s16)(e->world_y + e->hb_y);\n')
        c.append('            if (!ngpng_rects_overlap(ax, ay, s->hb_w, s->hb_h, bx, by, e->hb_w, e->hb_h)) continue;\n')
        c.append('            ngpng_player_shot_kill(shots, active_count, i);\n')
        c.append('            if (s->damage >= e->hp) e->hp = 0u;\n')
        c.append('            else if (e->hp > 0u) e->hp = (u8)(e->hp - s->damage);\n')
        c.append('            e->vx = ngpng_add_s8_clamped(e->vx, s->kb_x);\n')
        c.append('            e->vy = ngpng_add_s8_clamped(e->vy, s->kb_y);\n')
        c.append('            /* Non-fatal hit feedback — hit flash if enemy survived AND type opted in. */\n')
        c.append('            if (e->hp > 0u && sc->type_hit_flash) {\n')
        c.append('                u8 hflen;\n')
        c.append('                hflen = sc->type_hit_flash[e->type];\n')
        c.append('                if (hflen > 0u) e->hit_flash = hflen;\n')
        c.append('            }\n')
        if _has_dmg_popup:
        ...
        c.append('    if (*active_count == 0u) return;\n')
        c.append('    for (i = 0; i < (u8)NGPNG_AUTORUN_MAX_PLAYER_SHOTS; ++i) {\n')
        c.append('        /* PERF: cache &shots[i] in a local pointer — see comment in\n')
        c.append('         * the has_enemy variant. Decls without init at top of for-body. */\n')
        c.append('        NgpngPlayerShot *s;\n')
        c.append('        s16 sx_chk;\n')
        c.append('        s16 sy_chk;\n')
        c.append('        u8 _bn_hit;\n')
        ...
        c.append('            continue;\n')
        c.append('        }\n')
        c.append('        if (s->active) ngpng_player_shot_sync(sc, shots, i, 0u, cam_px, cam_py);\n')
        c.append('        if (processed >= *active_count) break;\n')
        c.append('    }\n')
        c.append('}\n')
        c.append('\n')
    if has_bullets:
        ...
        c.append('        /* PERF: cache &enemies[i] in a local pointer (cc900 ABI codegen). */\n')
        c.append('        NgpngEnemy *e;\n')
        c.append('        u8 etype;\n')
        c.append('        u8 btype;\n')
        c.append('        u8 fcond;\n')
        ...
        c.append('        e = &enemies[i];\n')
        c.append('        processed = (u8)(processed + 1u);\n')
        c.append('        etype = e->type;\n')
        ...
        c.append('            s16 dx = (s16)(player_world_x - e->world_x);\n')
        c.append('            s16 dy = (s16)(player_world_y - e->world_y);\n')
        ...
        c.append('            if (e->world_x < player_world_x) {\n')
        c.append('                if (processed >= enemy_active_count) break;\n')
        c.append('                continue;\n')
        c.append('            }\n')
        c.append('        }\n')
        c.append('        if (e->fire_cd > 0u) {\n')
        c.append('            e->fire_cd = (u8)(e->fire_cd - 1u);\n')
        c.append('            if (processed >= enemy_active_count) break;\n')
        c.append('            continue;\n')
        c.append('        }\n')
        ...
        c.append('            ngpng_enemy_bullet_spawn(shots, shot_active_count, shot_alloc_idx, btile, bpal, bflags, box, boy, hb_x, hb_y, hb_w, hb_h, damage, kb_x, kb_y, bvx, bvy, bbflags, bbsfx, e->world_x, e->world_y, player_world_y, cam_px, cam_py);\n')
        c.append('        }\n')
        ...
        c.append('        }\n')
        c.append('        e->fire_cd = type_fire_rate ? type_fire_rate[etype] : 40u;\n')
        c.append('        if (processed >= enemy_active_count) break;\n')
        c.append('    }\n')
        c.append('}\n')
        c.append('\n')
    ...
    if has_shooting:
        ...
        c.append("    if (shot->visible) ngpc_sprite_hide(slot);\n")
        c.append("    shot->visible = 0u;\n")
        c.append("    shot->last_sx = (s16)-32768;\n")
        c.append("    shot->last_sy = (s16)-32768;\n")
        c.append("}\n\n")
        c.append("static void ngpng_player_shot_sync(const NgpSceneDef *sc, NgpngPlayerShot *shots, u8 idx, u8 bullet_type, s16 cam_px, s16 cam_py)\n{\n")
        c.append("    NgpngPlayerShot *shot = &shots[idx];\n")
        c.append("    u8 slot = (u8)(NGPNG_AUTORUN_SHOT_SPR_BASE + idx);\n")
    ...
    # BG restoration on unpause is done via g_ngp_scenes[cur_scene].enter().
    # scene_X_blit_tilemaps() is static in scenes_autogen.c (not visible here),
    # so the only reliable cross-TU path is the enter() function pointer.
    ...
    if has_mapstream:
        ...
        for _ms in mapstream_scenes:
            ...
            # Propagate scene loop flags so mapstream wraps tile indices and
            # normalises camera-wrap deltas instead of clamping to MAX_DELTA
            # (which left huge black gaps when the camera wrapped around).
            c.append(f"        s_ms.loop_x = (u8){_SYM}_LOOP_X;\n")
            c.append(f"        s_ms.loop_y = (u8){_SYM}_LOOP_Y;\n")
            c.append(f"        break;\n")
            c.append(f"#endif\n")
        c.append("    default:\n")
        c.append("        break;\n")
    ...
    c.append("    /* Snapshot of gameplay state taken on pause-enter and restored at\n")
    c.append("     * pause-exit. Covers regs that neither ngpng_update_plane_scroll nor\n")
    c.append("     * the scene's enter() callback touch — viewport (HUD letterbox), the\n")
    c.append("     * BG color used outside the tilemap window, and BG enable bit.\n")
    c.append("     * Keeps the pause overlay flexible across scrolling/static scenes,\n")
    c.append("     * different tilemap sizes, and inverted SCR1/SCR2 priority setups. */\n")
    c.append("    u8  s_pause_save_win_x = 0u;\n")
    c.append("    u8  s_pause_save_win_y = 0u;\n")
    c.append("    u8  s_pause_save_win_w = 160u;\n")
    c.append("    u8  s_pause_save_win_h = 152u;\n")
    ...
    if has_enemy:
        if has_waves:
            ...
            if _has_wave_scroll:
                ...
                c.append("    /* F1: if the scene starts at a non-zero camera position (custom\n")
                c.append("     * cam_tile, checkpoint warp, mid-scene re-entry), fast-forward\n")
                c.append("     * the cursor past every wave whose scroll trigger is already\n")
                c.append("     * behind us. Without this, the per-frame loop would burst-spawn\n")
                c.append("     * all of them on the first frame and overflow the enemy pool. */\n")
                c.append("    {\n")
                c.append("        u8 _ff_wm = s_ngpng_wave_mode[cur_scene];\n")
                c.append("        if (_ff_wm != 0u && g_ngp_scenes[cur_scene].wave_table && g_ngp_scenes[cur_scene].wave_table_n > 0u) {\n")
                c.append("            s16 _ff_pos = (_ff_wm == 1u) ? cam_px : cam_py;\n")
                c.append("            const NgpcWaveEntry *_ff_table = g_ngp_scenes[cur_scene].wave_table;\n")
                c.append("            u8 _ff_n = g_ngp_scenes[cur_scene].wave_table_n;\n")
                c.append("            while (g_wave_scroll_cursor < _ff_n && (s16)_ff_table[g_wave_scroll_cursor].delay <= _ff_pos) {\n")
                c.append("                g_wave_scroll_cursor = (u8)(g_wave_scroll_cursor + 1u);\n")
                c.append("            }\n")
                c.append("            wave_seq.next = g_wave_scroll_cursor;  /* keep triggers in sync */\n")
    ...
    c.append("         * Gated on !s_paused: while paused, the pause-enter block has zeroed\n")
    c.append("         * SCR1/SCR2 scroll registers so the menu sits at fixed screen coords;\n")
    c.append("         * re-applying the camera scroll each frame here would make the menu\n")
    c.append("         * drift with the level. RESUME re-runs enter()/mapstream which restores\n")
    c.append("         * gameplay scroll values, and this branch resumes normal updates. */\n")
    ...
    c.append("            /* Snapshot regs the menu is about to clobber, so the resume path\n")
    c.append("             * can restore them exactly — independent of whatever the scene's\n")
    c.append("             * enter() callback re-runs. Covers HUD letterboxing, BG color and\n")
    c.append("             * BG enable bit; scroll regs + SCR_PRIO are re-applied by\n")
    c.append("             * ngpng_update_plane_scroll on the first frame after resume. */\n")
    c.append("            s_pause_save_win_x   = HW_WIN_X;\n")
    c.append("            s_pause_save_win_y   = HW_WIN_Y;\n")
    c.append("            s_pause_save_win_w   = HW_WIN_W;\n")
    c.append("            s_pause_save_win_h   = HW_WIN_H;\n")
    c.append("            s_pause_save_scr_prio= HW_SCR_PRIO;\n")
    c.append("            s_pause_save_bg_ctl  = HW_BG_CTL;\n")
    c.append("            s_pause_save_bg_col0 = HW_PAL_BG[0];\n")
    c.append("            /* Atomic pause-enter, ordered to mirror menu_kuroi_dokutsu's clean\n")
    c.append("             * transition pattern:\n")
    c.append("             *   1. Hide all hardware sprites (player, enemies, FX) immediately.\n")
    c.append("             *   2. Disable the raster scroll table — otherwise the per-line\n")
    c.append("             *      raster ISR keeps overwriting HW_SCR{1,2}_OFS_X every line\n")
    c.append("             *      with the gameplay scroll values and the menu drifts.\n")
    c.append("             *   3. Reset viewport to full screen (any HUD letterbox window is\n")
    c.append("             *      gone for the duration of the menu).\n")
    c.append("             *   4. Force background colour to black so any tile outside the\n")
    c.append("             *      cleared region shows black instead of the previous BG.\n")
    c.append("             *   5. Lock plane priority: SCR1 in front of SCR2 so the menu text\n")
    c.append("             *      (drawn on SCR1) is visible regardless of prior dialog state.\n")
    c.append("             *   6. Zero both scroll registers so tile (0,0) maps to screen (0,0).\n")
    c.append("             *   7. Full-tilemap clear of both planes (32x32, not just the 20x19\n")
    c.append("             *      visible area) — the off-screen tiles still flicker through\n")
    c.append("             *      during the transition frame otherwise.\n")
    c.append("             *   8. Draw the menu palette + text last so it's the freshest data\n")
    c.append("             *      in vramq before VBlank flushes.\n")
    c.append("             * RESUME below calls enter()+mapstream which restore everything. */\n")
    c.append("            ngpc_sprite_hide_all();\n")
    c.append("            /* NOTE: per-line raster scroll table (ngpc_raster_clear_scroll)\n")
    c.append("             * is NOT cleared here — that module is trimmed from the build by\n")
    c.append("             * NGPNG_TRIM_UNUSED on autogen projects that do not use it.\n")
    c.append("             * Projects that wire per-line scroll must call it from their own\n")
    c.append("             * pause hook to avoid the per-line ISR overwriting these regs. */\n")
    c.append("            ngpc_gfx_set_viewport(0u, 0u, 160u, 152u);\n")
    c.append("            ngpc_gfx_set_bg_color(RGB(0, 0, 0));\n")
    c.append("            HW_SCR_PRIO = 0x00u;\n")
    c.append("            ngpc_gfx_scroll(GFX_SCR1, 0u, 0u);\n")
    c.append("            ngpc_gfx_scroll(GFX_SCR2, 0u, 0u);\n")
    c.append("            ngpc_gfx_clear(GFX_SCR1);\n")
    c.append("            ngpc_gfx_clear(GFX_SCR2);\n")
    ...
    c.append("                 * BG color and BG enable bit. The scene's enter() (called below)\n")
    c.append("                 * rebuilds tiles+palette but is not guaranteed to touch these,\n")
    c.append("                 * and ngpng_update_plane_scroll only handles scroll + SCR_PRIO. */\n")
    c.append("                HW_WIN_X = s_pause_save_win_x;\n")
    c.append("                HW_WIN_Y = s_pause_save_win_y;\n")
    c.append("                HW_WIN_W = s_pause_save_win_w;\n")
    c.append("                HW_WIN_H = s_pause_save_win_h;\n")
    ...
    c.append("                s_paused = 0u;\n")
    c.append("                /* Restore snapshotted regs (see PAD_A handler above). */\n")
    c.append("                HW_WIN_X = s_pause_save_win_x;\n")
    c.append("                HW_WIN_Y = s_pause_save_win_y;\n")
    c.append("                HW_WIN_W = s_pause_save_win_w;\n")
    c.append("                HW_WIN_H = s_pause_save_win_h;\n")
    ...
    if has_enemy:
        if has_waves:
            if _has_wave_scroll:
                ...
                c.append("                u8  _wsa, _prev_ac, _new_slot;\n")
                c.append("                s16 _ax, _ay;\n")
                c.append("                while (_we) {\n")
                c.append("                    _we_ent.type = _we->type; _we_ent.x = _we->x; _we_ent.y = _we->y; _we_ent.data = _we->data;\n")
                c.append("                    _wsa = sc->wave_spawn_anchor;\n")
                ...
                c.append("                    if (_wsa != 0u && enemy_active_count > _prev_ac) {\n")
                c.append("                        _new_slot = (enemy_alloc_idx == 0u) ? (u8)((u8)NGPNG_AUTORUN_MAX_ENEMIES - 1u) : (u8)(enemy_alloc_idx - 1u);\n")
                c.append("                        _ax = 0; _ay = 0;\n")
                c.append("                        if (_wsa == 1u)      { _ax = (s16)(cam_px + 160); _ay = cam_py; }\n")
                c.append("                        else if (_wsa == 2u) { _ax = (s16)(cam_px - (s16)enemies[_new_slot].hb_w); _ay = cam_py; }\n")
                c.append("                        else if (_wsa == 3u) { _ax = cam_px; _ay = (s16)(cam_py - (s16)enemies[_new_slot].hb_h); }\n")
                c.append("                        else if (_wsa == 4u) { _ax = cam_px; _ay = (s16)(cam_py + 152); }\n")
                c.append("                        enemies[_new_slot].world_x = (s16)(enemies[_new_slot].world_x + _ax);\n")
                c.append("                        enemies[_new_slot].world_y = (s16)(enemies[_new_slot].world_y + _ay);\n")
                c.append("                    }\n")
                ...
                c.append("                {\n")
                c.append("                    NgpngEnt _we_ent;\n")
                c.append("                    u8  _wsa2, _prev_ac2, _new_slot2;\n")
                c.append("                    s16 _ax2, _ay2;\n")
                ...
                c.append("                        _wsa2 = sc->wave_spawn_anchor;\n")
                c.append("                        _prev_ac2 = enemy_active_count;\n")
                c.append("                        ngpng_enemy_spawn(sc, enemies, &enemy_active_count, &enemy_alloc_idx, &_we_ent, 0xFFu, 0xFFu, 0xFFu, 0u);\n")
                c.append("                        if (_wsa2 != 0u && enemy_active_count > _prev_ac2) {\n")
                c.append("                            _new_slot2 = (enemy_alloc_idx == 0u) ? (u8)((u8)NGPNG_AUTORUN_MAX_ENEMIES - 1u) : (u8)(enemy_alloc_idx - 1u);\n")
                c.append("                            _ax2 = 0; _ay2 = 0;\n")
                c.append("                            if (_wsa2 == 1u)      { _ax2 = (s16)(cam_px + 160); _ay2 = cam_py; }\n")
                c.append("                            else if (_wsa2 == 2u) { _ax2 = (s16)(cam_px - (s16)enemies[_new_slot2].hb_w); _ay2 = cam_py; }\n")
                c.append("                            else if (_wsa2 == 3u) { _ax2 = cam_px; _ay2 = (s16)(cam_py - (s16)enemies[_new_slot2].hb_h); }\n")
                c.append("                            else if (_wsa2 == 4u) { _ax2 = cam_px; _ay2 = (s16)(cam_py + 152); }\n")
                c.append("                            enemies[_new_slot2].world_x = (s16)(enemies[_new_slot2].world_x + _ax2);\n")
                c.append("                            enemies[_new_slot2].world_y = (s16)(enemies[_new_slot2].world_y + _ay2);\n")
                c.append("                        }\n")
                c.append("                        g_wave_scroll_cursor = (u8)(g_wave_scroll_cursor + 1u);\n")
                c.append("                    }\n")
                c.append("                }\n")
                c.append("                /* B1 fix: mirror the cursor into wave_seq.next so TRIG_WAVE_GE\n")
                c.append("                 * and TRIG_WAVE_CLEARED keep working in scroll mode — they read\n")
                c.append("                 * next_wave from wave_seq.next regardless of scheduling mode. */\n")
                c.append("                wave_seq.next = g_wave_scroll_cursor;\n")
                c.append("                g_wave_scroll_last_pos = _pos;\n")
                c.append("            }\n")
                c.append("        }\n")
            else:
                # Original frame-based path with wave_spawn_anchor support.
                c.append("        {\n")
                c.append("            const NgpcWaveEntry *_we = ngpc_wave_update(&wave_seq);\n")
                c.append("            NgpngEnt _we_ent;\n")
                c.append("            u8  _wsa, _prev_ac, _new_slot;\n")
    ...
    if has_triggers:
        ...
        if has_dialogues:
            ...
            c.append("                }\n")
            c.append("            }\n")
            # TRIG_ACT_SET_NPC_DIALOGUE (74): fired by TRIG_NPC_TALKED_TO (cond=36).
            # t->a0 = dialogue sequence index. Behaviour identical to SHOW_DIALOGUE.
            c.append("            if (t->action == TRIG_ACT_SET_NPC_DIALOGUE) {\n")
            c.append("                if (!(dialog_scene < (u8)NGP_SCENE_COUNT && ngpc_dlg_is_active(&dialog_runner))) {\n")
            c.append("                    ngpng_dialog_abort(&dialog_runner, &dialog_scene);\n")
            c.append("                    dialog_scene = cur_scene;\n")
            c.append("                    ngpng_dialog_open_for_scene(cur_scene, &dialog_runner, t->a0, cam_px, cam_py);\n")
