...
def _write_scene_bg_map_c(
    ...
    lines: list[str] = [
        ...
        f"/* FAR tileword array for ngpc_mapstream SCR1 ({map_w}x{map_h} tiles). */\n",
...
def write_scene_loader_h(
    *,
    ...
    if sprite_names or tilemap_infos:
        lines.append("\n")
    ...
    # SCR1 large map → handled exclusively by ngpc_mapstream (no dual-stream conflict).
    # stream_planes must NOT write SCR1 tiles — it would overwrite ngpc_mapstream's work
    # and double VRAM bandwidth, causing Character Over glitches on real hardware.
    scr1_by_mapstream = needs_stream.get("scr1", False)
    # Track A: a bg_chunk_map grid always produces a bg_map.c → always mapstream.
    if _bg_chunk_grid_paths:
        scr1_by_mapstream = True
    # Only SCR2 (if large) still uses stream_planes with direct VRAM writes.
    # Small maps (≤32×32) use hardware scroll register wrapping with no VRAM rewrites.
    any_streaming = needs_stream.get("scr2", False)
    ...
    if scr1_by_mapstream and _scr1_cam_map_w > 20 and _scr1_cam_map_h > 19:
