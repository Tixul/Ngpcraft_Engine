...
def _to_wine_path(path: str) -> str:
    """Convert a Linux absolute path to a Wine Windows path via winepath."""
    try:
...
def cmd_compile(src: str, obj: str, extra_flags: list[str]) -> int:
    ...
    use_wine = not _IS_WINDOWS and len(cc900_cmd) >= 2

    def _p(path: str) -> str:
        return _to_wine_path(path) if use_wine else path
    ...
    include_flags = [
        "-I" + _p(os.path.abspath(os.path.join(project_root, d)))
        for d in ("src", "src/core", "src/gfx", "src/fx", "src/audio")
    ]
    abs_extra_flags = []
    for flag in extra_flags:
        if flag.startswith("-I") and not os.path.isabs(flag[2:]):
            abs_extra_flags.append("-I" + _p(os.path.abspath(os.path.join(project_root, flag[2:]))))
        elif flag.startswith("-I") and os.path.isabs(flag[2:]):
            abs_extra_flags.append("-I" + _p(flag[2:]))
        else:
            abs_extra_flags.append(flag)
    cmd = cc900_cmd + ["-c", "-O3"] + include_flags + abs_extra_flags + [_p(src_abs), "-o", _p(obj_abs)]
    result = subprocess.run(
        cmd,
        cwd=cc900_dir,
        check=False,
