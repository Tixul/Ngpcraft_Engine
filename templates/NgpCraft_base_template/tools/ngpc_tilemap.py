...
def format_c_source(
    ...
    # Fallback for mono-color PNGs: when EVERY emitted palette entry is 0
    # (i.e. the source PNG only contained transparent + opaque-black pixels,
    # or any other single-color content that ends up encoded as 0x0000), the
    # tilemap would render invisible against the autorun's default black
    # bg_color. Replace the palette with a default text-style {black, white,
    # light-gray, dark-gray} so the BG is at least visible. Authors with real
    # multi-color PNGs are unaffected.
    if pal_words and all(w == 0 for w in pal_words):
        print(
            f"  WARN  {name}: palette extracted from PNG is uniformly black — "
            "substituting default text-style palette (transparent / black / "
            "light-gray / dark-gray) so the tilemap is visible. Edit the PNG "
            "to use real colors if you want custom hues.",
            file=sys.stderr,
        )
        default_pal = [0x0000, 0x0FFF, 0x0AAA, 0x0555]
        pal_words = []
        for pi in range(len(palette_colors)):
            pal_words.extend(default_pal)
...
def _format_layer_source(
    ...
    # Same fallback as the single-layer writer: mono-color PNG → default text palette.
    if pal_words and all(w == 0 for w in pal_words):
        print(
            f"  WARN  {prefix}: palette uniformly black — substituting default "
            "text-style palette (black / white / light-gray / dark-gray).",
            file=sys.stderr,
        )
        default_pal = [0x0000, 0x0FFF, 0x0AAA, 0x0555]
        pal_words = []
